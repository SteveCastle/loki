/*
 * slangfx studio — AI rotoscoping, prompt engine (model layer only).
 *
 * Wraps a SAM-family segmentation model (MobileSAM by default) running in
 * the browser through onnxruntime-web. This is the PROMPTED counterpart to
 * roto.js's BiRefNet backend: where BiRefNet mattes the most salient
 * subject with no guidance, SAM segments the SPECIFIC object you point at
 * (clicks, strokes, boxes, or a text query through an open-vocabulary
 * detector) and app.js tracks it across frames. The split follows
 * samexporter's export format, verified against the shipped defaults:
 *
 ...
 * localStorage['lowkey-studio.roto-sam-config'] to self-host.
 */
...
const DEFAULTS = {
  // The bundle (native-EP asyncify) build: SAM's small graphs run fine on
  // it. Do NOT share this config with roto.js — BiRefNet needs the JSEP
  // build, and each engine keeps its own ort module instance.
...
// NOT the old 'lowkey-studio-roto-v1' — roto.js best-effort deletes that
// SAM-era cache on load (it predates the engine split); this name is safe.
const CACHE_NAME = 'lowkey-studio-roto-sam-v1';

function rotoConfig() {
  try {
    return { ...DEFAULTS, ...JSON.parse(localStorage.getItem('lowkey-studio.roto-sam-config') ?? '{}') };
  } catch {
    return { ...DEFAULTS };
  }
}
 ...
 * models kept re-downloading in the app. Same origin, so no CORS. */
