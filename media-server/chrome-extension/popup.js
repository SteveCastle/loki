...
async function populateCurrentUrl() {
  try {
    ...
    if (tab?.url) {
      currentUrl = tab.url;
      elements.urlDisplay.textContent = truncateUrl(tab.url);
      elements.urlDisplay.title = tab.url;
    } else {
...
async function loadPreferences() {
  return new Promise((resolve) => {
    chrome.storage.local.get(['ingestOptions'], (result) => {
      if (result.ingestOptions) {
        elements.optTranscript.checked =
          result.ingestOptions.transcript || false;
