...
func buildFacesServeArgs(detectorPath, recognizerPath, ortLib string, m FaceModel, provider string, threads int) []string {
	args := []string{
		"--faces",
		"--serve",
...
func facesTask(j *jobqueue.Job, q *jobqueue.Queue, mu *sync.Mutex) error {
	...
	scanned, facesFound, autoAssigned, skipped, err := runFacesPool(ctx, j, q, paths, fromQuery, model, detectorPath, recognizerPath, embedBin)
	if err != nil {
		if ctx.Err() != nil {
...
// skipped).
func runFacesPool(ctx context.Context, j *jobqueue.Job, q *jobqueue.Queue, paths []string, fromQuery bool, model FaceModel, detectorPath, recognizerPath, embedBin string) (int, int, int, int, error) {
	...
	baseArgs := buildFacesServeArgs(detectorPath, recognizerPath, ortLib, model, provider, threads)
